...
from halow.ippo.buffer import RolloutBuffer
import torch
import torch.nn
...
class Ippo:
    """Independent PPO — each agent has its own actor AND its own critic.
    
    The only difference from MAPPO is:
    - Two independent critics, each taking the agent's LOCAL observation as input
      and outputting a scalar value (instead of one centralized critic taking
      global state and outputting values for all agents).
    - No global state is used during training.
    """
    ...
    def __init__(self, env: CustomEnvironment):
        ...
        # critics — each takes local obs and outputs a SCALAR value (not per-agent)
        self.drone_critic = Critic(self.obs_dim, 1, self.hidden_dim, self.num_layers).to(self.device)
        self.rover_critic = Critic(self.obs_dim, 1, self.hidden_dim, self.num_layers).to(self.device)
        ...
        self.drone_critic_optim = Adam(self.drone_critic.parameters(), lr=self.learning_rate_critic)
        self.rover_critic_optim = Adam(self.rover_critic.parameters(), lr=self.learning_rate_critic)
    ...
    def train(self, runs=10):
        ...
        for run in range(runs):
            ...
            while num_episode < self.num_episodes_per_batch:
                ...
                while not terminated and not truncated:
                    with torch.no_grad():
                        ...
                        # IPPO: each critic takes its own agent's local observation
                        drone_value = self.drone_critic(drone_obs).squeeze(-1)   # scalar
                        rover_value = self.rover_critic(rover_obs).squeeze(-1)   # scalar
                        value = torch.stack([drone_value, rover_value])           # [2]
                ...
                if truncated:
                    ...
                    with torch.no_grad():
                        drone_obs = torch.tensor(obs["drone"], device=self.device)
                        rover_obs = torch.tensor(obs["rover"], device=self.device)
                        drone_final = self.drone_critic(drone_obs).squeeze(-1)
                        rover_final = self.rover_critic(rover_obs).squeeze(-1)
                        episode["final_value"] = torch.stack([drone_final, rover_final]).cpu().numpy()
            ...
            (batch_obs, batch_action_masks, batch_actions, batch_log_probs, batch_returns, batch_adv) = buf.batchify()
            drone_critic_losses = []
            rover_critic_losses = []
            drone_actor_losses = []
            rover_actor_losses = []
            ...
            for i in range(self.epochs):
                ...
                drone_critic_grad_norm = None
                rover_critic_grad_norm = None
                
                for start in range(0, batch_obs.size(0), self.batch_size):
                    end = start + self.batch_size
                    
                    # ── drone critic ──
                    self.drone_critic_optim.zero_grad()
                    drone_obs = batch_obs[start:end, 0, :]
                    drone_pred_values = self.drone_critic(drone_obs).squeeze(-1)      # [B]
                    drone_target_returns = batch_returns[start:end, 0]                 # [B]
                    drone_critic_loss = torch.nn.functional.mse_loss(drone_pred_values, drone_target_returns)
                    drone_critic_loss.backward()
                    drone_critic_losses.append(drone_critic_loss.item())
                    drone_critic_grad_norm = torch.nn.utils.clip_grad_norm_(self.drone_critic.parameters(), max_norm=self.max_grad_norm)
                    self.drone_critic_optim.step()
                    
                    # ── rover critic ──
                    self.rover_critic_optim.zero_grad()
                    rover_obs = batch_obs[start:end, 1, :]
                    rover_pred_values = self.rover_critic(rover_obs).squeeze(-1)      # [B]
                    rover_target_returns = batch_returns[start:end, 1]                 # [B]
                    rover_critic_loss = torch.nn.functional.mse_loss(rover_pred_values, rover_target_returns)
                    rover_critic_loss.backward()
                    rover_critic_losses.append(rover_critic_loss.item())
                    rover_critic_grad_norm = torch.nn.utils.clip_grad_norm_(self.rover_critic.parameters(), max_norm=self.max_grad_norm)
                    self.rover_critic_optim.step()
                    ...
                    drone_actor_loss = -drone_grad_loss - self.entropy_coefficient * drone_entropy_loss.mean()
            ...
            self.log_run(
                log_path, run,
                collected_episodes,
                drone_critic_losses, rover_critic_losses,
                drone_actor_losses, rover_actor_losses,
                drone_entropies, rover_entropies,
                drone_grad_norm.item(), rover_grad_norm.item(),  # type: ignore
                drone_critic_grad_norm.item(), rover_critic_grad_norm.item()  # type: ignore
            )       
    ...
    def save_policy(self, run: int):
        checkpoint = {
            ...
            "drone_critic": self.drone_critic.state_dict(),
            "rover_critic": self.rover_critic.state_dict(),
            "drone_actor_optim": self.drone_actor_optim.state_dict(),
            "rover_actor_optim": self.rover_actor_optim.state_dict(),
            "drone_critic_optim": self.drone_critic_optim.state_dict(),
            "rover_critic_optim": self.rover_critic_optim.state_dict(),
        }
    ...
    def setup_logger(self):
        ...
        with open(log_path, "w", newline="") as f:
            ...
            writer.writerow([
                ...
                "drone_entropy",
                "rover_entropy",
                "drone_actor_grad_norm",
                "rover_actor_grad_norm",
                "drone_critic_grad_norm",
                "rover_critic_grad_norm",
            ])
        return log_path


    def log_run(self, log_path, run, episodes,
                drone_critic_losses, rover_critic_losses,
                drone_actor_losses, rover_actor_losses,
                drone_entropies, rover_entropies,
                drone_grad_norm, rover_grad_norm,
                drone_critic_grad_norm, rover_critic_grad_norm):
        ...
        row = [
            ...
            np.mean(drone_critic_losses),
            np.mean(rover_critic_losses),
        ...
        print(
            f"[Run {run:>3}] "
            f"Return drone: {np.mean(episode_returns_drone):>8.3f} ± {np.std(episode_returns_drone):.3f} | "
            f"Return rover: {np.mean(episode_returns_rover):>8.3f} ± {np.std(episode_returns_rover):.3f} | "
            f"Ep length: {np.mean(episode_lengths):>6.1f} ± {np.std(episode_lengths):.1f} | "
            f"Drone critic: {np.mean(drone_critic_losses):>8.4f} | "
            f"Rover critic: {np.mean(rover_critic_losses):>8.4f} | "
            f"Drone actor: {np.mean(drone_actor_losses):>8.4f} | "
            f"Rover actor: {np.mean(rover_actor_losses):>8.4f} | "
            f"Entropy — drone: {np.mean(drone_entropies):.3f} rover: {np.mean(rover_entropies):.3f}"
        )
